			...
			if (event.keyCode != 13 && event.keyCode != 9)
			{
				...
				else
				{
					...
					timeout = setTimeout(function() {
						...
						// Get the JSON data.
						$.getJSON(options.src+"&core_aj=1&action=suggest&qq="+q, function(data, textStatus) {
										...
										if (options.showEmptyCategories || (!options.showEmptyCategories && group['data'].length != 0))
										{
											...
											$.each(group['data'], function (j, item) {
												if (count < limit)
												{
													...
													output += '<td>';
													output += '';
													output += (item['primary'] != undefined) ? '<span class="'+options.classPrefix+'result-title">'+item['primary']+"</span>\n" : '';
